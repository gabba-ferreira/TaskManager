...
const getTasksById =  async(req,res) => {
    try{
        const task = await Task.findById(req.param._id);
        if(!task) return res.status(404).json({message:"Task not found", error:error.message});
...
const getUserDashboardData = async(req,res) => {
    try{
        ...
        const taskPrioritiesLevels = taskPriority.reduce((acc, priority) => {
...
module.exports = {getTasks,getTasksById,createTask,updateTask,deleteTask,updatedTaskStatus,updateTaskChecklist,getDashboardData,getUserDashboardData}